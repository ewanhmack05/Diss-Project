...
function SavedCountList() {
  ...
  if (editing) {
    return <SavedCountEdit cellCount={editing} onBack={() => setSelectedCellCountId(null)} />
  }
  ...
  if (status === 'error') {
    return <p className="saved-cell-count-list-empty">Couldn't reach the annotation store.</p>
  }
  ...
  return (
    <ul className="saved-cell-count-list">
          ...
          <li key={cellCount.id} className="saved-cell-count-list-row">
            ...
            >
              <CellCountColourSwatch
                breakdown={colourBreakdownFromDots(parseCellCountDots(cellCount.dots))}
                className="saved-cell-count-list-swatch"
              />
              <span className="saved-cell-count-list-label">{cellCount.label}</span>
              <span className="saved-cell-count-list-count">{cellCount.count}</span>
            </button>
            <button
              type="button"
              className={`saved-cell-count-list-view${isViewing ? ' saved-cell-count-list-view--active' : ''}`}
              disabled={cellCount.locationX === null || cellCount.locationY === null}
              title={
              ...
              onClick={() => setViewedCellCountId(toggleViewedCellCountId(viewedCellCountId, cellCount.id))}
            >
              {isViewing ? 'Hide' : 'View'}
            </button>
          </li>
